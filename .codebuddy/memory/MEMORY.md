...
- **增强终端开关**（2026-09-04 用户裁决，默认**关**）：工具栏 `ToolbarIcon`（Sparkles，与查找/历史/文件同款 active 高亮，右键画布菜单同步一项）。关着=纯终端：`planCommandSubmission(cmd, mode, {capture:false})` **不注入任何受控标记**、不出结果面板；只有打开时才注入 OSC133 并生成结果 Tab。三处必须同时门禁：提交计划传 `capture`、`onResult` 用 `enhancedRef` 守卫生结果、抽屉/结果按钮加 `enhancedTerminal &&`；关闭时清空结果。状态持久化 `localStorage` 键 `bls-ops.terminal.enhanced`。工具栏**已删除"编码"栏目**（后端会话编码 API 保留、默认 auto 不动）。
