...
/// caller must show them to the user and only reconnect once the key is trusted.
#[derive(Debug, Clone)]
pub enum ConnectOutcome {
    Connected { host_key: HostKeyInfo },
    HostKeyUnknown { host_key: HostKeyInfo, hop: String },
    HostKeyChanged { host_key: HostKeyInfo, known_fingerprint: String, hop: String },
}
...
pub fn parse_ssh_target(input: &str, default_port: u16) -> Result<(String, String, u16)> {
    ...
    let port = if port.is_empty() {
        ...
        port.parse::<u16>().map_err(|_| anyhow!("端口必须是 1 到 65535 之间的数字"))?
    };
    if port == 0 {
...
impl SshSessionManager {
        ...
        match connect_hop(&target, cols, rows).await? {
            HopResult::Connected(connection) => {
                ...
                self.sessions.lock().await.insert(
                    session_id,
                    SshSession { handle: connection.handle, writer, _chain: connection.chain },
                );
                Ok((ConnectOutcome::Connected { host_key }, Some(reader)))
            }
            HopResult::HostKeyUnknown { host_key, hop } => {
                Ok((ConnectOutcome::HostKeyUnknown { host_key, hop }, None))
            }
            HopResult::HostKeyChanged { host_key, known, hop } => Ok((
                ConnectOutcome::HostKeyChanged { host_key, known_fingerprint: known, hop },
                None,
            )),
        }
    }
    ...
    pub async fn input(&self, session_id: &str, data: Vec<u8>) -> Result<()> {
        ...
        let session = sessions.get(session_id).ok_or_else(|| anyhow!("SSH 会话不存在或已断开"))?;
        session.writer.data_bytes(data).await?;
        Ok(())
    }
...
enum HopResult {
    ...
    HostKeyUnknown { host_key: HostKeyInfo, hop: String },
    HostKeyChanged { host_key: HostKeyInfo, known: String, hop: String },
}
...
async fn connect_hop(target: &ConnectTarget, cols: u32, rows: u32) -> Result<HopResult> {
    ...
    let attempt = match target.proxy_jump.as_deref() {
        Some(jump) => {
            ...
            let channel = jump_handle
                .channel_open_direct_tcpip(target.host.clone(), target.port as u32, "127.0.0.1".to_string(), 0)
                .await?;
    ...
    if !authenticated.success() {
        return Err(anyhow!("{}@{} 认证失败：用户名或凭据被拒绝", target.username, target.label()));
    }
    ...
    Ok(HopResult::Connected(HopConnection { handle, channel, host_key, chain }))
}
...
async fn host_key_outcome(
    ...
    match target.known_fingerprint.clone() {
        None => Some(HopResult::HostKeyUnknown { host_key, hop: target.label() }),
        Some(known) if known != host_key.fingerprint => {
            Some(HopResult::HostKeyChanged { host_key, known, hop: target.label() })
        }
        Some(_) => None,
    }
}
...
impl client::Handler for ClientHandler {
    ...
    fn check_server_key(
        ...
        async move {
            *observed.lock().await = Some(HostKeyInfo { fingerprint: fingerprint.clone(), key_type });
