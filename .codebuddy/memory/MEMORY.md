...
- **GitHub REST `GET /releases/tags/{tag}` 不返回 draft**（tauri-action 建的就是 draft）：release 查询/轮询一律用 `GET /releases` 列表 + `jq 'map(select(.tag_name==…))[0]'`；资产下载 `gh api .../releases/assets/<id> -H "Accept: application/octet-stream"`；`gh release upload --clobber` 对 draft 可用。`gh api` 失败仍会写 HTTP body 进重定向文件——用 `jq -e` 验语义别只看文件大小。draft 的 manifest 在 Publish 前对应用不可见（`/releases/latest/download/` 忽略 draft）。
