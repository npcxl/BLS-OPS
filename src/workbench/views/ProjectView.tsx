...
import { opsApi, toErrorMessage, type AdapterReadiness, type ProjectCandidate, type ProjectScanResult, type ProjectScanStatus, type ServerCapabilityProfile } from "@/api/ops-api";
...
export function ProjectView({ tab }: { tab: WorkspaceTab }) {
  ...
  useEffect(() => () => { if (timerRef.current !== null) window.clearInterval(timerRef.current); unlistenRef.current?.(); if (scan && ["running", "queued"].includes(scan.state)) void opsApi.projectScanCancel(scan.id); }, [scan]);
  ...
  return (
    <ModuleFrame tab={tab} session={session} icon={FolderSearch} toolbar={<>
      ...
      <div className="flex flex-col gap-4 p-4">
        <div className="flex flex-wrap items-center gap-1.5 rounded-[9px] border border-line bg-surface-1 px-3 py-2 text-10 text-fg-subtle shadow-sm"><span className="rounded bg-accent/12 px-2 py-1 text-accent">1 发现项目</span><span>→</span><span>2 确认项目与模块</span><span>→</span><span>3 检查部署环境</span><span>→</span><span>4 生成部署方案（P4）</span></div>
        {error && <div className="rounded-[8px] border border-danger/30 bg-danger/10 px-3 py-2 text-12 text-danger">{error}</div>}
        {!result && !loading && <div className="flex flex-col items-center justify-center gap-3 px-6 py-12 text-center"><ModuleEmpty icon={FolderSearch} title="发现服务器项目" hint="先识别服务器的操作系统与已安装能力，再据此启用对应的收集器（systemd、Docker、Nginx、PM2 等均按需），最后结合 Git、进程、端口与配置线索定位项目。整个过程只读，不会执行部署。" /><Button variant="primary" size="sm" disabled={!session.ready} onClick={() => void discover()}>开始发现</Button></div>}
        {loading && scan && <ScanProgress scan={scan} onCancel={() => void cancel()} />}
        {result && <>
          {result.capability && <CapabilityGraph profile={result.capability} />}
          ...
          {result.deployment_readiness.length > 0 && <ReadinessGraph items={result.deployment_readiness} />}
...
function CapabilityGraph({ profile }: { profile: ServerCapabilityProfile }) {
  ...
  return (
    <section className="rounded-[10px] border border-line bg-surface-1 p-3">
      <div className="mb-2 flex items-center gap-1 text-11 font-medium text-fg"><span className="rounded bg-accent/12 px-2 py-0.5 text-accent">服务器能力图谱</span><span className="text-fg-subtle">先识别服务器，再决定启用哪些收集器</span></div>
      ...
      {missing.length > 0 && <div className="mt-2 text-10 text-fg-subtle">未安装（不启用收集器，避免无意义报错）：{missing.join("、")}</div>}
    </section>
  );
}

const READINESS_LABEL: Record<string, string> = { ready: "可直接部署", needs_install: "需安装", conflict: "冲突", unconfirmed: "无法确认" };

function ReadinessGraph({ items }: { items: AdapterReadiness[] }) {
  const verdictClass = (v: string) => v === "ready" ? "bg-success/12 text-success" : v === "needs_install" ? "bg-warning/12 text-warning" : v === "conflict" ? "bg-danger/12 text-danger" : "bg-surface-3 text-fg-subtle";
  return (
    <section className="rounded-[10px] border border-line bg-surface-1 p-3">
      <div className="mb-2 text-11 font-medium text-fg"><span className="rounded bg-accent/12 px-2 py-0.5 text-accent">部署可行性图谱</span></div>
      <div className="grid grid-cols-2 gap-1.5 md:grid-cols-3">
        {items.map((item) => (
          <div key={item.adapter} className="flex items-center justify-between gap-2 rounded-[6px] border border-line px-2 py-1.5">
            <span className="truncate text-11 text-fg">{item.adapter}</span>
            <span className={cn("shrink-0 rounded px-1.5 py-0.5 text-10", verdictClass(item.verdict))}>{READINESS_LABEL[item.verdict] ?? item.verdict}</span>
          </div>
        ))}
      </div>
      <div className="mt-2 text-10 text-fg-subtle">未确认的部署方式不会猜测为支持；需安装的方式由 P4 决定是否处理。</div>
    </section>
  );
}
