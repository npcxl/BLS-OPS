...
- **提示符精简是会话级开关（设置→终端，默认关）**：`terminal-prompt.ts` 的注入行 `[ -n "$BASH_VERSION" ] && PS1='\[\e]7;file://\h$PWD\a\]\W\$ '`，**只在连接成功那一瞬间发一次**（绝不监听设置变化即时注入——那会往用户正敲的命令行里插字）。三个要点勿改：`$BASH_VERSION` 门控（不认识的 shell 一个字都不写）、`\[ \]` 包非打印序列（否则 readline 光标算错）、**顺带上报 OSC 7**（用户看 `\W`，我们拿权威 cwd）。服务器 `.bashrc` 方案仍是给用户的可选替代（所有客户端生效）。
